...
tmp="${env_file}.tmp"
grep -v -E '^export (VOICE_PLAYBACK_PATH|VOICE_SPK_VOLUME|VOICE_CHANNEL|VOICE_GAIN_DB|VOICE_SECONDS|SKIP_WELCOME)=' "$env_file" > "$tmp"
cat >> "$tmp" <<'EOF'
...
grep -E '^export (VOICE_PLAYBACK_PATH|VOICE_SPK_VOLUME|VOICE_CHANNEL|VOICE_GAIN_DB|VOICE_SECONDS|SKIP_WELCOME)=' "$env_file"
